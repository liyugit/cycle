...
baidu.dom.ready(
	function(){
			...
			var dragNow = function(target,option){
					...
					baidu.array.each(baidu.dom.query("#friendNotInCycle .selected"),function(item,i){
						selectedInfo[item.id] = item.id;
						if(item != target){
							setStyles(item,{left:targetPosition.left+"px",top:targetPosition.top+"px"});
							baidu.dom.addClass(item,"transform");
						}
					});
					//碰撞检测，是否拖入圈子
			...
			cycle.init = function(){
				...
				baidu.array.each(baidu.dom.query("#friendNotInCycle .not-friend"),function(item,i){
					var obj = item;
					friendNotInCycleList[obj.id].position = baidu.dom.getPosition(obj);
					//绑定点击事件
					baidu.on(obj,"mousedown",function(e){
					if(baidu.dom.hasClass(obj,"selected")){
